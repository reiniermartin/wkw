...
var Ball = {
  new: function (incrementedSpeed) {
    return {
      width: this.canvas.height / 30,
      height: this.canvas.height / 30,
      x: (this.canvas.width / 2) - (this.canvas.height / 60),
      y: (this.canvas.height / 2) - (this.canvas.height / 60),
...
var Game = {
  initialize: function () {
    ...
    this.ai.speed = setPaddleSpeed() * .9;
  ...
  update: function () {
      if (!this.over && playingPong) {
        ...
        if (this.ball.x - (this.ball.width/4) <= this.player.x && this.ball.x >= this.player.x - this.player.width) {
          if (this.ball.y <= this.player.y + this.player.height && this.ball.y + this.ball.height >= this.player.y) {
            this.ball.x = (this.player.x + this.ball.width);
            this.ball.moveX = DIRECTION.RIGHT;
            var audio = new Audio('https://cdn.jsdelivr.net/gh/reiniermartin/wkw@main/sounds/hit.mp3');
            audio.play();
          }
        }
        ...
        if (this.ball.x <= 0) {
          var audio = new Audio('https://cdn.jsdelivr.net/gh/reiniermartin/wkw@main/sounds/comScore.mp3');
          audio.play();
        }
        if ((this.ball.x) >= (this.canvas.width - this.ball.width )) {
          var audio = new Audio('https://cdn.jsdelivr.net/gh/reiniermartin/wkw@main/sounds/userScore.mp3');
          audio.play();
        }
        if ((this.ball.y) <= (0) || (this.ball.y) >= (this.canvas.height - this.ball.height)) {
          var audio = new Audio('https://cdn.jsdelivr.net/gh/reiniermartin/wkw@main/sounds/wall.mp3');
          audio.play();
        }
      }
