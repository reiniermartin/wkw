...
var DIRECTION = {
    ...
    ballImage = new Image();
    ballImage.src = 'https://uploads-ssl.webflow.com/64ecc781e5f35e8100018546/65b1465ac1f7e282059f3cf2_wkw-ball.svg';
...
var Ball = {
    new: function (incrementedSpeed) {
        return {
            width: this.canvas.height / 20,
            height: this.canvas.height / 20,
            x: (this.canvas.width / 2) - (this.canvas.height / 40),
            y: (this.canvas.height / 2) - (this.canvas.height / 40),
...
var Game = {
    ...
    update: function () {
        if (!this.over && playingPong) {
            ...
            if (this.ball.x <= this.player.x && this.ball.x >= this.player.x - this.player.width) {
...
Pong.initialize();
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // easter egg trigger
    if (typeof RapidClickOrTapDetector === 'undefined') {
        class RapidClickOrTapDetector {
            constructor(clickThreshold = 4, timeLimit = 800) {
            ...
            start() {
                document.addEventListener('click', this.handleInputBound);
                document.addEventListener('touchend', this.handleInputBound);
            }
            stop() {
                document.removeEventListener('click', this.handleInputBound);
                document.removeEventListener('touchend', this.handleInputBound);
            }
            handleInput() {
                const now = Date.now();
                this.clickTimes.push(now);
                while (this.clickTimes.length > 0 && now - this.clickTimes[0] > this.timeLimit) {
                    this.clickTimes.shift();
                }
                if (this.clickTimes.length === this.clickThreshold) {
                    this.onRapidClicksOrTapsDetected();
                    this.clickTimes = [];
                }
            }
            onRapidClicksOrTapsDetected() {
                if (pongGameElement) {
        ...
        const rapidClickOrTapDetector = new RapidClickOrTapDetector();
        rapidClickOrTapDetector.start();
    }
